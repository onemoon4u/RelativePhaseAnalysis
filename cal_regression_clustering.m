   %% To calcluate the cluster indices 'k' for each frame of the Relative Phase Movie 
    %
    % beta: the coefficients of linear regression done across frames of Relative Phase Movies 
    % unit: time resolution of input data, should be in second
    % n_nulls: number of random rotations for the null-model
    % IDX_in: not required for K=4. This will be utilized for K other than 4.
    % 
    % IDX: the clustered results.
    % w_prop: transition matrix from a null-model.
    % 
    % In the transition matrix, (i,j) means the transition from mode j to mode i.
    %
    % made by Youngjai Park,
    % modified by Joon-Young Moon
    %
    % Modification for current version: 2025-8-01
    %
    % This code will be modified for K>4 in ther near future.
